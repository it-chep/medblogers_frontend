...
export const MyCheckbox: FC<IProps & PropsWithChildren> = ({label, checked, onSelected = () => {}, checkBoxTop, children}) => {
    ...
    return (
        <label className={classes.myCheckbox + (checkBoxTop ? ` ${classes.top}` : '')} onMouseDown={cancelSelection}>
            ...
            <span className={classes.checkboxText}>{label || children}</span>
        </label>
    )
}